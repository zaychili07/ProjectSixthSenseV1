...
Do not copy internal labels without explaining them.
Do not simply repeat CRITICAL, re-escalation reason, or critical reason.
Translate the labels into plain clinical-style language.
Use the actual numeric values from the patient facts.
...
- Mention the risk trend.
- Mention the abnormal vital signs by name and value.
- Mention the meaningful lead time if present.
- Explain that CRITICAL means BIRE detected failure to stabilize after the event.
- Explain that this is not a diagnosis or treatment recommendation.

Output:

1. Clinical summary:
BIRE is showing this patient because

2. Current concern:
The current concern is supported by

3. Timeline interpretation:
The timeline shows

4. Post-event interpretation:
After deterioration onset, BIRE observed

5. What to review:
A clinician may want to review

6. Safety note:
This explanation is generated by
"""
    return prompt.strip()
